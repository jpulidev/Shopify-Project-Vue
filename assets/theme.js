...
theme.Cart = (function() {
  ...
  Cart.prototype = Object.assign({}, Cart.prototype, {
    ...
    _createBackUpCart: function(cartTable) {
      ...
      cartTable
        ...
        .forEach(function(lineItem) {
          ...
          //save the max quantity value
          var quantityInput = lineItem.querySelector(
            "[data-cart-quantity-wrapper]"
          ).dataset.limitProduct;
          if (!quantityInput) {
            quantityInput = "";
          }
    ...
    _restoreBackupCart: function(cartBackup, state) {
      var cartIndex = 1;
      cartBackup = cartBackup.filter(checkForRemoved);

      function checkForRemoved(item) {
        if (state.items.find((o) => o.variant_id == item.variant_id[0])) {
          return item;
        }
      }
      ...
      cartBackup.forEach(function(lineItem, index) {
        ...
        if (itemKeyVariantId && itemKeyVariantId[0] != lineItem.variant_id[0]) {
          return;
        }

        // when the matching cart item is found, restore the additional cart pieces
        if (lineItem.product_link !== "") {
          findMatchingItem.setAttribute(
            "data-cart-item-url",
            lineItem.product_link
          );
        }
        ...
        var linesContainer = findMatchingItem.querySelector("[data-type-info");
        if (linesContainer) {
        ...
        if (lineItem.quantity_input != "") {
          var findQuantity = findMatchingItem.querySelector(
            "[data-cart-quantity-wrapper]"
          );

          if (findQuantity) {
            findQuantity.setAttribute(
              "data-limit-product",
              lineItem.quantity_input
            );

            var desktopQuantityInput = findQuantity.querySelector(
              "input[data-quantity-input]"
            );
            var mobileQuantityInput = findQuantity.querySelector(
              "input[data-quantity-input-mobile]"
            );
            if (desktopQuantityInput) {
              desktopQuantityInput.max = lineItem.quantity_input;
            }
            if (mobileQuantityInput) {
              mobileQuantityInput.max = lineItem.quantity_input;
            }
          }
        }
        ...
        if (lineItem.has_addon !== "") {
          ...
          document
            .querySelector(
              "[data-for-product='" + lineItem.variant_id[0] + "']"
            )
            .setAttribute("data-cart-item-index", cartIndex);
        }
    ...
    _createCart: function(state) {
      ...
      this._restoreBackupCart(cartBackup, state);
